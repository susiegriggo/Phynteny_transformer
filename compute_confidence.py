...
@click.option('--output', 'output_path', type=click.Path(), help='Path to save the output confidence dictionary.')
@click.option('--device', default='cpu', help='Device to use for computation (default: cpu)')
def main(model_directory, embeddings_path, categories_path, integer_category_path, output_path, device):
    logger.info("Starting the computation of confidence scores.")
    embeddings, categories, phrog_integer = load_data(embeddings_path, categories_path, integer_category_path)
    p = create_predictor(model_directory, device)
    conf_dataset_loader = create_dataloader(embeddings, categories)
...
def load_data(embeddings_path, categories_path, integer_category_path):
    ...
    return embeddings, categories, phrog_integer
...
def create_dataloader(embeddings, categories):
    logger.info("Creating the dataset and dataloader.")
    conf_dataset = model_onehot.VariableSeq2SeqEmbeddingDataset(
        list(embeddings.values()), list(categories.values()), mask_portion=0)
    conf_dataset.set_validation(list(categories.values()))
