...
def main(input_data, gene_categories, maximum_genes, prefix, out, exclude_embedding, extra_features): 
    ...
    if extra_features: 
        X, y = format_data.process_data(embeddings, data, exclude_embedding = exclude_embedding) 
            
    else:
        X, y = format_data.process_data(embeddings, data, extra_features = False, exclude_embedding = exclude_embedding) 
