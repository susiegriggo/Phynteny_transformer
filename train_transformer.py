"""
Script for training transformer model 
""" 
...
@click.option('--lr', default=1e-5, help='Learning rate for the optimizer.')
@click.option('--epochs', default=5, help='Number of training epochs.')
@click.option('--hidden_dim', default=512, help='Hidden dimension size for the transformer model.')
@click.option('--num_heads', default=4, help='Number of attention heads in the transformer model.') 


def split(genomes, plm_vectors, plm_integer, test_size=0.2): 
    """
    ...
    for g in genomes[:500]: 
        ...
        #this_vectors = [torch.tensor(plm_vectors.get(i), dtype=torch.float16) for i in this_genes]
        this_vectors = [plm_vectors.get(i) for i in this_genes]
        
        # get the corresponding functions 
        this_categories = [plm_integer.get(i) if plm_integer.get(i) != None else -1 for i in this_genes]
        
        # keep the information for the genomes that are not entirely unknown 
        if all(element == -1 for element in this_categories):
            removed.append(g) 
        else: 
            # store the info in the dataset a
            X.append(torch.tensor(this_vectors))
...
def main(batch_size, lr, epochs, hidden_dim, num_heads): 
    ...
    plm_vectors = pickle.load(open('data/phrogs_genomes/pLM_embs.pkl', 'rb'))
    plm_ogg = pickle.load(open('data/phrogs_genomes/ogg_assignment.pkl', 'rb'))  
    ...
    # Create model 
    model = model.VariableSeq2SeqTransformerClassifier(input_dim=1280, num_classes=10, num_heads=num_heads, hidden_dim=hidden_dim) 

    # Train the model 
    model.train(model, train_dataloader, epochs=epochs, lr=lr) 

    # Evaluate the model 
    model.evaluate_with_threshold(model, test_dataloader, threshold=0.8) 
