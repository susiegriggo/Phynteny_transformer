...
def main(pharokka_x_path, pharokka_y_path, phold_y_path, model_dir, output_dir):
    ...
    mean_tpr_dict = {}
    mean_auc_dict = {}
    std_tpr_dict = {}  # Add this line

    for i in range(num_classes):
        mean_tpr_dict[i] = np.mean(fpr_dict[i], axis=0)
        std_tpr_dict[i] = np.std(fpr_dict[i], axis=0)  # Add this line
    ...
    with open(f'{output_dir}/std_tpr_dict.pkl', 'wb') as f:  # Add this block
        pickle.dump(std_tpr_dict, f)

    logger.info("ROC data saved successfully.")
