...
class VariableSeq2SeqEmbeddingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Randomly mask some of the input embeddings
        if self.training:
            masked_embedding = self.random_mask(embedding, category)
        else:
            masked_embedding = embedding

        return masked_embedding, category, mask 
    
    def random_mask(self, embedding, category):
        """
        Select a single instance to mask in each input
        """
        
        print('randomly masking', flush = True)
        ...
        masked_embedding[idx][0] = self.mask_token
...
def train(model, train_dataloader, test_dataloader, epochs=5, lr=1e-5, save_path='model', device='cuda'):
    ...
    for epoch in range(epochs):
        ...
        for embeddings, categories, masks in train_dataloader:
            ...
            optimizer.zero_grad()
            src_key_padding_mask = (masks == 0)  # Mask for transformer - refers to the masks tensor which is composed of 1s and 0s 
...
def train_crossValidation(dataset, phrog_integer, n_splits=10, batch_size=16, epochs=10, lr=1e-5, save_path='out', num_heads=4, hidden_dim=512, device='cuda', dropout=0.1): 
    ...
    for train_index, val_index in kf.split(dataset):
        ...
        input_dim = dataset.embeddings[0].shape[1] 
